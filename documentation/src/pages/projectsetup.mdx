...
For cloudflare dns Setup you need to have cloudflare account. You can ignore this if you do not plan to use `inbound_rues` in local setup. so you can ignore the `CLOUDFLARE_EMAIL`, `CLOUDFLARE_API_KEY` & `CLOUDFLARE_ZONE_ID` variables in `.env` as well.
